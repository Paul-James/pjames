...
makepath <- function(
    ...
    suppressMessages(require(pjames))
    ...
    pathVec <- function(KeyX, subset = subset){
        ...
        ## Feature Request: CODE USED FOR TIME ORDERED/SENSITIVE PATHWAYS FOR BPCI PROJ
        ## la <- split(teste, teste$epi.id)
    ...
    # parallelize it
    pathH <- hash(keys(personH), mclapply(keys(personH),
                                          function(X) pathVec(X, subset = subset),
                                          mc.cores = detectCores()))

    # assign path values to the path vector
    df_path <- data.frame(id = keys(pathH), path = values(pathH))
