---
title: "Paul's Personal Package Panacea"
output: 
    github_document:
        toc: true
---
...
Replace **[replace-me-(brackets-as-well)-with-PAT]** with the token I give you and restart the R session (which reloads the R environment variables).
...
1. in `R`, change the working directory to the root directory of the package repo
1. create the documentation using `document()` from the `devtools` package
1. use `git` to `commit` and `push` function and documentation changes as well as `git add` to add new functions to the repo
1. use the `devtools::install_github()` command to reinstall the package
1. if using Rstudio, refresh packages/documentation
